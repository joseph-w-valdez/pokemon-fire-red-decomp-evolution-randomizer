...
- **Generic UI chip painter (TypeIcon becomes a preset)** — generalize today’s blank-pill path into a reusable chip painter that can draw **any** small chrome shape (arbitrary W×H silhouette, corner policy, fill / two-tone, optional label + white-ink leftover-pad center, same-pal vs mixed surround/`Commit`). The current type pill is then a thin **preset** on top of that chip API (stock bake + blank/`HP ` FIT stay as TypeIcon recipes that *consume* the generic chip). Mirror in tools: `preview_type_pills.py` → e.g. `preview_ui_chip.py` (shape + label + palette in; type sheet compare = one recipe). Goal: new badges/bars stop re-learning the same GBA constraints; keep bake blits where pixel-identical vanilla still matters.
- **Debug component gallery** — DEBUG MENU page that shows framed panel + slider + radar presets (`PREVIEW` / `MEDIUM` / `LARGE`) + portrait (+ chip presets when the above lands), ideally with live nudge/scale tweaks to cut rebuild loops
